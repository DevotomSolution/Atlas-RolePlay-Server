                ...
                <div class="newphone__maps_client">
                    <div class="box-between">
                        <div class="box-column">
                            <div class="greenid">Nr {order.uid}</div>
                            <div class="box-flex">
                                <div class="newmphone__client_circle mech"></div>
                                <div>{order.area}</div>
                            </div>
                            <div class="gray">Distanţă {order.dist} м.</div>
                            <div>Bunuri: <span class="violet">{order.name}</span></div>
                            <div>Vei primi: <span class="green">${format("money", order.price)}</span></div>
                        </div>
                        <div class="phoneicons-Button newphone__chevron-back"></div>
                    </div>
                    <div class="newphone__client_button" on:click={() => onTakeOrder (order.uid)}>Ia ordine</div>
                </div>
            {/each}
            <div class="newphone__project_button" on:click={() => onLoad (10 + countLoad)}>Încărcați comenzi</div>
        {:else}
            <div class="newphone__forbes_anon newphone__project_padding20">
                <div class="newphone__forbes_hidden noneelements"></div>
                <div class="gray">Nu există comenzi active. Dar în curând va apărea ceva..</div>
            </div>
        {/if}
    </div>
</div>