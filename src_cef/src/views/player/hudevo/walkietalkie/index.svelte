<script>
    ...
    const changeWalkieTalkieVolume = (value) => {
        if (new Date().getTime() - eventsRefresh < 500) {
            window.notificationAdd(4, 9, 'Prea repede', 1000);
            return false;
        }
    ...
    const changeFrequency = () => {
        ...
        if (new Date().getTime() - eventsRefresh < 1000) {
            window.notificationAdd(4, 9, 'Prea repede', 1500);
            return;
        }
    ...
    const swapChannel = (state) => {
        if (isGovFactionPlayer === false) {
			window.notificationAdd(4, 9, 'Valul general este disponibil numai pentru stat. fracțiunile', 1500);
            return;
        }
