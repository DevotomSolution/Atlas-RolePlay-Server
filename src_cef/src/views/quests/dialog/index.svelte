<script>
    ...
    const handleMouseMove = () => {
        // Face interfața vizibilă și o ascunde
        // după 2,5 secunde de inactivitate
    ...
    const clickButton = (item, startCutScene = true) => {
        if (item.questId) {
            if (item.type === "perform") { //Mai întâi, deschideți dialogul cu recompensa
    ...
    /**
     * aName (string) - Biblioteca de bot
     * qID (int) - Numărul misiunii
     * status (bool) - A fost acceptată misiunea?
     * compility (int) - Misiunea este finalizată (0 - Dacă nu | 1... - Dacă este finalizată)
     */ 

    const showDialogQuests = (aName, qId, status, compility) => {
        questData.select = 0;
        questData.actorName = aName;
        if (qId === -1) { //Dacă linia de misiuni nu este disponibilă
            questData.data = getQuest(aName, -1);
        } else if (status === 0) {//Dacă linia de căutare a fost acceptată, dar actorul nu a fost încă vorbit
            questData.data = getQuest(aName, qId);
        } else if (status === 1 && compility == 0) { //Dacă ați vorbit deja cu actorul, dar misiunea nu este finalizată
            questData.data = getQuest(aName, qId);
        ...
        } else if (status === 1 && compility != 0) { //Dacă ați vorbit deja cu actorul, dar misiunea este finalizată
            questData.data = getQuest(aName, qId);
    ...
    <div class="questsnewdialog-main">
        ...
        <div class="questsnewdialog-answers" in:fly={{x: 200, duration: 1000}}>
            <div class="questsnewdialog-answer active answer-title">
               <div class="box-column">
                    ...
                    <!--<div class="questsnewdialog-condition">
                        <span class="questsnewdialogicon-star icon-condition"/> Nivelul 3 necesar
                    </div>-->
               </div>
            ...
            <!--<div class="questsnewdialog-answer">
               <div class="box-column">
                    <div class="questsnewdialog-subtitle">Ascultă, sunt de acord, când ar trebui să încep? Te avertizez, am o armă</div>
                    <div class="questsnewdialog-condition">
                        <span class="questsnewdialogicon-star icon-condition"/> Nivelul 3 necesar
                    </div>
               </div>
            ...
            <div class="questsnewdialog-answer">
                <div class="box-column">
                     <div class="questsnewdialog-subtitle">Spune-ne mai multe despre caz</div>
                </div>
                <span class="questsnewdialogicon-loop icon-answer"/>
             </div>
             <div class="questsnewdialog-answer exit">
                <div class="box-column">
                     <div class="questsnewdialog-subtitle">Sunt ocupat</div>
                </div>
